...
// Pool dan fungsi khusus untuk label angka sumbu koordinat (MathJax overlay)
const labelPoolSumbu = {};

function tampilkanLabelSumbu(key, x, y, texInner, alignH = "center", alignV = "middle") {
    const tex = `\\(${texInner}\\)`;
    let entry = labelPoolSumbu[key];
    if (!entry) {
        const el = document.createElement("div");
        el.className = "sumbu-label";
        ...
        labelOverlay.appendChild(el);
        entry = labelPoolSumbu[key] = { el, lastTex: null };
    }

    entry.el.style.display = "block";

    let translateX = "0%", translateY = "0%";
    if (alignH === "center") translateX = "-50%";
    else if (alignH === "right") translateX = "-100%";
    if (alignV === "middle") translateY = "-50%";
    else if (alignV === "bottom") translateY = "-100%";

    entry.el.style.left = x + "px";
    entry.el.style.top = y + "px";
    entry.el.style.transform = `translate(${translateX}, ${translateY})`;

    if (entry.lastTex !== tex) {
        entry.lastTex = tex;
        entry.el.innerHTML = tex;
        if (window.MathJax && MathJax.typesetPromise) {
            MathJax.typesetPromise([entry.el]).catch(() => {});
        }
    }
...
function drawCartesian() {
    ...
    if (showCoords) {
        ...
        for (let x = startGridX; x < width; x += gridSize) {
            ...
            if (mathX !== 0) {
                tampilkanLabelSumbu("sx-" + mathX, x, labelYOffset, formatPecahanLatex(mathX), "center", "top");
            }
        }
        ...
        for (let y = startGridY; y < height; y += gridSize) {
            ...
            if (mathY !== 0) {
                tampilkanLabelSumbu("sy-" + mathY, labelXOffset, y, formatPecahanLatex(mathY), "right", "middle");
            }
        }
        if (originX >= 0 && originX <= width && originY >= 0 && originY <= height) {
            tampilkanLabelSumbu("s0", originX - 6, originY + 6, "0", "right", "top");
        }
    }
    ...
    // Sembunyikan label sumbu yang tidak dirender ulang (misal setelah pan/zoom)
    Object.keys(labelPoolSumbu).forEach(key => {
        if (!labelKeysAktif.has(key)) {
            labelPoolSumbu[key].el.style.display = "none";
        }
    });
